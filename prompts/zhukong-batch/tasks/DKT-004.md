请进入主控模式，执行任务 DKT-004（Implement strict plan compiler）。
...
【任务锚点】
- Task ID: DKT-004
- Title: Implement strict plan compiler
...
【Concrete Actions】
1) Build compiler with deterministic TASK_ID and RUN_ID
2) Enforce required fields goal/actions/acceptance/expected_outputs/dependencies
3) Add duplicate and contradiction checks

【Acceptance Criteria】
1) Compiler rejects malformed or under-specified steps
2) Output stable across repeated runs
3) Compiler output directly consumable by dispatch engine

【Allowed Scope（超出即拒绝）】
- src/planner/ src/contracts/ tests/planner/
