请进入主控模式，执行任务 DKT-020（Put controller into subagent lane ownership）。
...
【任务锚点】
- Task ID: DKT-020
- Title: Put controller into subagent lane ownership
- Dependencies: DKT-019

【Goal】
Represent controller authority as subagent-lane ownership, not external window ownership.

【Concrete Actions】
1) Integrate controller lane ownership into runtime dispatch path
2) Persist controller ownership via existing lease and lifecycle structures
3) Keep external relay side read-only for status and forwarding

【Acceptance Criteria】
1) Active run records controller lane ownership in lease and lifecycle views
2) Routing ownership visible in status without schema break
3) Execution remains compatible with current CLI interface

【Allowed Scope（超出即拒绝）】
- src/orchestrator/ src/dispatch/ src/reliability/lease/ src/state/ tests/orchestrator/ tests/reliability/ tests/cli/
