请进入主控模式，执行任务 DKT-048（Define heartbeat lease takeover observability diagnostics model）。
...
【任务锚点】
- Task ID: DKT-048
- Title: Define heartbeat lease takeover observability diagnostics model
- Dependencies: DKT-047
...
2) Define takeover reason timing and correlation outputs
3) Define operator-consumable timeline views

【Acceptance Criteria】
1) Diagnostics schema covers heartbeat lease and takeover lifecycle
2) Correlation model links task run and step entities
3) Model is usable by report and dashboard layers

【Allowed Scope（超出即拒绝）】
- src/reliability/ src/contracts/ tests/reliability/ docs/workflows/ docs/reports/
