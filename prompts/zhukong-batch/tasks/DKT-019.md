请进入主控模式，执行任务 DKT-019（Enforce observer relay boundary）。
...
【任务锚点】
- Task ID: DKT-019
- Title: Enforce observer relay boundary
- Dependencies: DKT-018

【Goal】
Implement explicit relay-only behavior for the external main window.

【Concrete Actions】
1) Introduce relay-mode policy and guard abstraction
2) Block direct execution-role operations in relay mode
3) Add relay allow/deny tests

【Acceptance Criteria】
1) Relay mode rejects direct execution actions deterministically
2) Required relay context fields are preserved
3) CLI command and argument names remain unchanged

【Allowed Scope（超出即拒绝）】
- src/orchestrator/ src/dispatch/ src/state/ tests/orchestrator/ tests/cli/ docs/observer-relay-*.md
