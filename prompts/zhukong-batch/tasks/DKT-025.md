请进入主控模式，执行任务 DKT-025（Produce pure Chinese README with language switch）。
...
【任务锚点】
- Task ID: DKT-025
- Title: Produce pure Chinese README with language switch
- Dependencies: DKT-024

【Goal】
Provide a Chinese-only readme with clear navigation back to English version.

【Concrete Actions】
1) Create README.zh-CN.md as pure Chinese narrative
2) Add top-level language switch link to English readme
3) Align section mapping with English readme

【Acceptance Criteria】
1) README.zh-CN.md is Chinese-first narrative only
2) Language switch links back to English readme
3) Section parity and command accuracy match English readme

【Allowed Scope（超出即拒绝）】
- README.zh-CN.md README.md docs/
