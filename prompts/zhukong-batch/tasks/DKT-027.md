请进入主控模式，执行任务 DKT-027（Add runnable collaboration example and docs wiring）。
...
【任务锚点】
- Task ID: DKT-027
- Title: Add runnable collaboration example and docs wiring
- Dependencies: DKT-026

【Goal】
Ship runnable example and integrate bilingual links across entry docs.

【Concrete Actions】
1) Add CLI example script for observer-relay collaboration flow
2) Wire links from readmes and workflow overview docs
3) Validate doc navigation and command reproducibility

【Acceptance Criteria】
1) Example script demonstrates expected recovery chain
2) Readmes and workflow docs link to the runnable example
3) English and Chinese navigation is complete and consistent

【Allowed Scope（超出即拒绝）】
- examples/cli/ README.md README.zh-CN.md docs/workflows/ docs/backend-to-agent-workflows.md
