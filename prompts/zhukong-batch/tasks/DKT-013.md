请进入主控模式，执行任务 DKT-013（Implement heartbeat daemon and status evaluator）。
...
【任务锚点】
- Task ID: DKT-013
- Title: Implement heartbeat daemon and status evaluator
- Dependencies: DKT-005

【Goal】
Detect stalled long-running execution using explicit and implicit signals.
...
【Allowed Scope（超出即拒绝）】
- src/reliability/heartbeat/ src/state/ tests/reliability/
