请进入主控模式，执行任务 DKT-021（Implement self-healing closed loop）。
...
【任务锚点】
- Task ID: DKT-021
- Title: Implement self-healing closed loop
- Dependencies: DKT-020

【Goal】
Implement deterministic detect/decide/takeover/recover behavior for controller continuity.

【Concrete Actions】
1) Encode decision policy using heartbeat and lease signals
2) Integrate takeover and handoff apply flows for recovery
3) Emit compatible diagnostic events with regression tests

【Acceptance Criteria】
1) WARNING does not force takeover
2) STALE or invalid lease triggers takeover path
3) Recovery updates succession metadata with schema compatibility

【Allowed Scope（超出即拒绝）】
- src/reliability/heartbeat/ src/reliability/lease/ src/reliability/succession/ src/reliability/handoff/ src/orchestrator/ tests/reliability/ tests/cli/
