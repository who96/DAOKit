请进入主控模式，执行任务 DKT-023（Add compatibility guardrails and rollback runbook）。
...
【任务锚点】
- Task ID: DKT-023
- Title: Add compatibility guardrails and rollback runbook
- Dependencies: DKT-022

【Goal】
Guarantee non-breaking delivery for CLI/contracts/release evidence anchors.

【Concrete Actions】
1) Add CLI parser surface regression checks
...
【Acceptance Criteria】
1) CLI commands and argument names are unchanged
2) Schema invariants pass without enum or version break
3) Rollback runbook is actionable and test-linked

【Allowed Scope（超出即拒绝）】
- src/cli/ contracts/ tests/cli/ tests/contracts/ docs/ docs/reports/final-run/
