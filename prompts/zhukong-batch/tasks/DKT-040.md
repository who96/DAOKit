请进入主控模式，执行任务 DKT-040（Implement criterion mapping diagnostics output）。
...
【任务锚点】
- Task ID: DKT-040
- Title: Implement criterion mapping diagnostics output
...
【Allowed Scope（超出即拒绝）】
- src/verification/ src/contracts/ tests/verification/ docs/reports/
