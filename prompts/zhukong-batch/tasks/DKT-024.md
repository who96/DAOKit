请进入主控模式，执行任务 DKT-024（Produce pure English README with language switch）。
...
【任务锚点】
- Task ID: DKT-024
- Title: Produce pure English README with language switch
- Dependencies: DKT-023

【Goal】
Provide an English-only readme with clear navigation to Chinese version.

【Concrete Actions】
1) Rewrite README.md as pure English narrative
2) Add top-level language switch link to Chinese readme
3) Keep quickstart commands and docs links valid

【Acceptance Criteria】
1) README.md has no Chinese narrative text
2) Language switch links correctly to Chinese readme
3) Quickstart and docs map remain valid
