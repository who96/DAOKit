请进入主控模式，按“波次并行 + worktree 隔离”执行 v1.1 Tooling/Verification Hardening 任务。

【项目根目录】
/Users/huluobo/workSpace/DAOKit

【前置条件】
- DKT-028 ~ DKT-037 已全部验收 SUCCESS。
- v1.1 资产基线已存在：specs/004-v1-1-tooling-verification-hardening/*
...
【执行波次（必须按波次推进）】
- Wave 0（串行）: DKT-038
- Wave 1（并行）: DKT-039 + DKT-040
- Wave 2（并行）: DKT-041 + DKT-043 + DKT-044
- Wave 3（串行）: DKT-042
- Wave 4（串行）: DKT-045
- Wave 5（串行）: DKT-046
