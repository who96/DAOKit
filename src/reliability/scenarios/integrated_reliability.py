...
def run_integrated_reliability_scenario(
    *,
    ...
    # Persist operator-facing recovery report derived from authoritative state store
    reports_dir = scenario_root / "reports"
    op_json = reports_dir / "operator-recovery.json"
    op_md = reports_dir / "operator-recovery.md"
    try:
        op_payload, op_artifacts = build_and_persist_operator_recovery_report(
            task_id=task_id,
            run_id=run_id,
            state_root=scenario_root / "state",
            output_json=op_json,
            output_markdown=op_md,
        )
        operator_recovery = {
            "json": str(op_json),
            "markdown": str(op_md),
            "evidence_pointers": op_payload.get("evidence_pointers", {}),
        }
    except Exception:  # pragma: no cover - diagnostics integration is best-effort
    ...
    return {
        ...
        "operator_recovery_outputs": operator_recovery,
        "command_log": [
            {
